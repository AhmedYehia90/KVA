...
function optionalText(fd:FormData,key:string){const v=fd.get(key);return typeof v==="string"&&v.trim()?v.trim():null}
...
export async function setPilotItemUnlockAction(fd:FormData){
  const user=await requireOperationsConsoleAdmin();const admin=createAdminClient();const itemId=fd.get("itemId");
  if(typeof itemId!=="string"||!itemId)redirect(url("error","A valid Pilot Marketplace item is required."));
  try{
    const {error}=await admin.rpc("set_pilot_marketplace_unlock_requirements",{
      p_item_id:itemId,
      p_minimum_career_xp:amount(fd,"minimumCareerXp"),
      p_minimum_flights:amount(fd,"minimumFlights"),
      p_required_rank_code:optionalText(fd,"requiredRankCode"),
      p_required_milestone_code:optionalText(fd,"requiredMilestoneCode"),
      p_actor_id:user.id,
      p_organization_id:ORG
    });if(error)throw error;
  }catch(e){redirect(url("error",e instanceof Error?e.message:"Unable to update Pilot Marketplace unlock requirements."))}
  revalidatePath("/operations/economy");revalidatePath("/pilot/economy");redirect(url("message","Pilot Marketplace unlock requirements updated."));
}
